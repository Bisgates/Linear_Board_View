...
- [2026-05-19 17:30] v0.33.7 — ADD ISSUE / WORKING ON / CUSTOM 的 ▾ 换成 stroke chevron
  - unicode "▾" 是黑色实心三角，跟旁边 uppercase 11px label 视觉权重不搭；改成 10px 矩形 viewBox 内 1.4px stroke chevron，opacity 0.65 跟 paper 颜色调和

- [2026-05-19 17:25] v0.33.6 — WORKING ON / CUSTOM tab 中间分隔点单独画大一号
  - middle dot 跟 label 拆成两个 span，dot fontSize 10 → 14、垂直对中、opacity 0.45，跟 uppercase title 视觉上对齐

- [2026-05-19 17:20] v0.33.5 — WORKING ON 后缀宽度 140 → 100，原本留太多空白

- [2026-05-19 17:15] v0.33.4 — pinned chip 拖排序换 pointer events 实现
  - HTML5 DnD 在 Tauri WebKit 上被 OS-level 文件拖入服务拦截不工作；改成 setPointerCapture + 4px threshold 自己实现拖动，单击仍 activate，超过阈值才进 drag

- [2026-05-19 17:05] v0.33.3 — issue count 固定宽度，0/1/N 切换不再带着中部 chip 抖动

- [2026-05-19 17:00] v0.33.2 — 顶栏 tab 不再随 d / 1-9 切 view 抖动 + pinned chip 拖排序修复
...
- [2026-05-19 16:55] v0.33.1 — drop 判定调成"放松"而非"收紧"
  - 上一版 0.33.0 把判定 bbox 缩到 60% 中心区，跟 user 要的"放松"方向相反；改 1.2 倍向外扩，蹭边也能 reparent

- [2026-05-19 16:50] v0.33.0 — 粘图卡更干净 + drop 光晕染目标色 + 拖判定收紧
...
- [2026-05-19 15:51] v0.32.0 — ⌘V 走 OS clipboard 单一路径 + 空格聚焦视觉中心 + drag-reparent 自动 tidy + drop cue 改光晕
...
- [2026-05-19 14:35] v0.31.0 — UI 调（zoom / 方向键 pan / ADD ISSUE 固定位）+ Shift+Tab 顺序 fix 二次修
...
- [2026-05-19 11:55] v0.30.0 — 拖卡到另一张卡上 → reparent
  - 拖动中目标卡边框出现暖红 cue；旧 parent edge 自动清掉，多选拖整组改嫁同一 target，按 U 一步还原 edge + 位置

- [2026-05-19 11:55] v0.29.0 — iCloud Drive 自动备份 + 一堆 bug 修
...
- [2026-05-19 11:43] v0.28.0 — Custom view 可右键 pin 到顶栏 chip 条 + 单键切换视图（A/S/D/1–9）
...
- [2026-05-15 17:33] v0.20.0 — Mindmap tidy 快捷键 (F / Shift+F) + 干净 edge 路由
  - F 整理 focused subtree、Shift+F 整画布；同源 edge 在 shared stem 处汇合

- [2026-05-15 16:51] v0.19.0 — NoteCard 双向链接 `[[YYMMDDxx]]` + 右键扩展菜单
