...
- [2026-07-14 09:19] v0.40.0 — 主题收敛为 Default 与 FigJam：Default 沿用现有 Figma 视觉；FigJam 提供六色居中便签、清晰框选与四角尺寸调整、可持久化缩放滚动和更贴近 FigJam 的连线交互，并移除便签待办状态，旧主题选择会自动迁移

- [2026-07-12 11:02] v0.39.0 — 窗口换成 macOS 原生 Tahoe 外观：更大的圆角、新版红绿灯浮在内容左上；标题栏透明、内容顶到窗口顶、去掉标题文字和下沿细线；顶栏空白处任意位置都能拖动窗口
  - 顺带修掉深色模式下窗口边缘的一圈黑边（窗体外观钉成浅色）

- [2026-07-12 10:15] v0.38.1 — FigJam 设为默认主题：新装/未选过主题时开局即为 FigJam 风格；已手动选过 Default 的不受影响

- [2026-07-10 06:12] v0.38.0 — 新增可切换主题：菜单里加 "Theme" 项，可在默认暖色和 FigJam 风格（浅灰画布、扁平淡蓝卡片、白色面板、蓝色高亮、灰色连线）之间切换，选择会持久化、重启保留
